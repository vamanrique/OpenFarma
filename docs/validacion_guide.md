...
Esta guía permite a cualquier evaluador verificar de forma independiente los resultados reportados por FarmaVigia.
...
### Qué verificar
Que la búsqueda en FarmaVigia retorna los mismos medicamentos que la fuente oficial.

### Pasos

1. Ir a [FarmaVigia](https://openfarma-production.up.railway.app) y buscar "ibuprofeno"
...
**Resultado esperado:** FarmaVigia mostrará un subconjunto deduplicado de los resultados del CUM (agrupa presentaciones del mismo principio activo + forma + concentración).
...
### Qué verificar
Que el estado INVIMA mostrado en FarmaVigia coincide con el último comunicado oficial.
...
2. Buscar un medicamento que aparezca en ese comunicado en FarmaVigia
3. Verificar que el estado (Monitorización / No desabastecimiento) coincide

**Limitación conocida:** La base de datos de FarmaVigia se actualiza cuando el script `actualizar_invima.py` se ejecuta (proceso manual o por cron los viernes). Puede haber hasta 7 días de diferencia con el comunicado más reciente.
...
cd backend
python retrain_invima.py --db farmavigia.db --eval-only
```
...
**Nota:** Los resultados pueden variar ligeramente si el INVIMA publicó nuevos PDFs desde que se generaron los resultados reportados. El script usa el historial disponible en `farmavigia.db`.
...
1. Buscar "metformina" en FarmaVigia
