...
GRUPO_LABEL: dict[str, str] = {
    "SOLIDO_ORAL":      "Solido oral",
    "SOLIDO_ORAL_LP":   "Liberacion prolongada",
    "ORAL_DISPERSABLE": "Dispersable",
    "LIQUIDO_ORAL":     "Liquido oral",
...
def _hydrate_group(grupo: GrupoEquivalencia, db: Session) -> GrupoDetalle:
    """Fetch product details for all cum_ids in a group."""
    ...
    for cum_id in cum_ids:
        ...
        if cum:
            ...
            # Product not found but still include the ID
            productos.append(ProductoEnGrupo(
                cum_id=cum_id,
                nombre_comercial=cum_id,
                laboratorio=None,
...
def get_grupos_equivalencia(cum_id: str, db: Session = Depends(get_db)):
    ...
    # 9. Hydrate groups (fetch product details)
    mi_grupo = _hydrate_group(mi_grupo_raw, db) if mi_grupo_raw else None
    misma_via = [_hydrate_group(g, db) for g in misma_via_raw]
    otras_vias = [_hydrate_group(g, db) for g in otras_vias_raw]
